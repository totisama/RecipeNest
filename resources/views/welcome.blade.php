    ...
    <div style="grid-template-columns: repeat(auto-fit, minmax(250px, 1fr));"
        ...
        @foreach ($recipes as $recipe)
            <a href="{{route('recipes.show', [$recipe->id])}}"
                class='group overflow-hidden select-none max-w-80 pb-3 flex flex-col items-center gap-4 bg-white w-full px-5 rounded-3xl transition-transform duration-300 ease-out h-full hover:cursor-pointer'>
                <img src="{{$recipe->image}}" alt="{{$recipe->title}} image"
                    class="rounded-3xl h-48 transition-transform duration-500 ease-in-out md:h-56 group-hover:scale-105" />
